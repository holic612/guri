...
def get_current_price(ticker):
    """현재가 조회"""
    return pybithumb.get_orderbook(ticker=ticker)["orderbook_units"][0]["ask_price"]
    ...
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=10):
            ...
            if target_price < current_price:
                krw = get_balance("KRW")
                if krw > 5000:
                    bithumb.buy_market_order("BTC", krw*0.9995)
        else:
            btc = get_balance("BTC")
            if btc > 0.00008:
                bithumb.sell_market_order("BTC", btc*0.9995)
        time.sleep(1)
